...
version_name="${MODRINTH_VERSION_NAME:-$version_number}"
changelog="${MODRINTH_CHANGELOG:-Automated $MODRINTH_VERSION_TYPE build.}"
...
if [[ "$MODRINTH_DRY_RUN" == "true" ]]; then
  ...
  jq '{project_id, name, version_number, version_type, game_versions, loaders, dependencies}' "$metadata_file"
  exit 0
fi
