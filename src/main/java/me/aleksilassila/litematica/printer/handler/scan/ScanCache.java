...
public final class ScanCache {
    ...
    public void clear() {
        ...
        lastFilterStats = new FilterStats(0, 0, 0, -1);
    ...
    /** 索引区块过滤器的最新统计快照,供 HUD 显示过滤状态 */
    public record FilterStats(int totalBuckets, int includedBuckets, int skippedBuckets, int chunkRadius) {
        public boolean isFilterActive() {
            return this.chunkRadius >= 0;
        }
    }

    private static volatile FilterStats lastFilterStats = new FilterStats(0, 0, 0, -1);

    public static FilterStats getLastFilterStats() {
    ...
    private static boolean containsAny(List<PrinterBox> boxes, int x, int y, int z) {
        for (PrinterBox box : boxes) {
    ...
    private final class SectionScanSession {
        ...
        private DistanceCursor distanceCursor;
        ...
        private SectionScanSession(
            ...
            this.distanceCursor = createDistanceCursor(this.sourceBoxes, region);
        }
        ...
        void updateRegion(SectionRegion region, List<PrinterBox> sourceBoxes) {
            ...
            boolean centerChanged = !this.region.sameCenterSection(region);
        ...
        private void resetProgress() {
            ...
            this.distanceCursor = createDistanceCursor(this.sourceBoxes, this.region);
        ...
        /**
         * 根据原理图索引是否可用,创建合适的距离游标.
         * 索引就绪且与当前原理图匹配时,使用 IndexDistanceCursor 直击非空气位置;
         * 否则回退到 PlayerDistanceCursor 做空间遍历.
         */
        private DistanceCursor createDistanceCursor(List<PrinterBox> boxes, SectionRegion region) {
            // 索引仅包含原理图非空气坐标,只对 PRINT 意图有效;
            // MINE/BEDROCK 等需要真实世界方块,不能用索引,否则会漏掉真实世界的目标
            if (this.intent == ScanIntent.PRINT
                    && SchematicBlockIndex.INSTANCE.isReady()
                    && SchematicBlockIndex.INSTANCE.getTotalNonAir() > 0) {
                Long2ObjectOpenHashMap<LongOpenHashSet> buckets =
                        SchematicBlockIndex.INSTANCE.getChunkBuckets();
                if (buckets != null && !buckets.isEmpty()) {
                    int chunkRadius;
                    if (Configs.Print.PRINT_INDEX_CHUNK_FILTER.getBooleanValue()) {
                        int workRange = Configs.Core.WORK_RANGE.getIntegerValue();
                        chunkRadius = (workRange + 15) / 16 + 1;
                    } else {
                        chunkRadius = -1;
                    }
                    // 统计过滤情况,供 HUD 显示
                    ...
                    int playerChunkX = region.centerSectionX();
                    int playerChunkZ = region.centerSectionZ();
                    for (Long2ObjectOpenHashMap.Entry<LongOpenHashSet> entry : buckets.long2ObjectEntrySet()) {
                        LongOpenHashSet bucket = entry.getValue();
                        ...
                        if (chunkRadius >= 0) {
                            ...
                            int bucketChunkX = chunkKeyX(chunkKey);
                            int bucketChunkZ = chunkKeyZ(chunkKey);
                            if (Math.abs(bucketChunkX - playerChunkX) > chunkRadius
                                    || Math.abs(bucketChunkZ - playerChunkZ) > chunkRadius) {
                                continue;
                            }
                        }
                        includedBuckets++;
                    }
                    ScanCache.lastFilterStats = new FilterStats(
                            totalBuckets, includedBuckets, totalBuckets - includedBuckets, chunkRadius
                    );
                    return new IndexDistanceCursor(
                            buckets,
                            boxes,
                            region.centerX(),
                            region.centerY(),
                            region.centerZ(),
                            region.centerSectionX(),
                            region.centerSectionZ(),
                            chunkRadius
                    );
                }
            }
            ScanCache.lastFilterStats = new FilterStats(0, 0, 0, -1);
            return new PlayerDistanceCursor(boxes, region);
        }
    ...
    private interface DistanceCursor {
    ...
    /** 从 ChunkPos 编码的 long 值中提取 chunkX (低 32 位) */
    private static int chunkKeyX(long chunkKey) {
        return (int) chunkKey;
    }

    /** 从 ChunkPos 编码的 long 值中提取 chunkZ (高 32 位) */
    private static int chunkKeyZ(long chunkKey) {
        return (int) (chunkKey >>> 32);
    }

    /**
     * 基于原理图非空气方块索引的距离游标.
     * 直接遍历索引中的非空气位置,按到玩家的平方距离排序,避免对稀疏原理图的逐方块空间扫描.
     */
    private static final class IndexDistanceCursor implements DistanceCursor {
        private final PriorityQueue<IndexNode> pending = new PriorityQueue<>();
        private final LongOpenHashSet emitted = new LongOpenHashSet();
        private final List<PrinterBox> boxes;
        private boolean complete;

        private IndexDistanceCursor(
                Long2ObjectOpenHashMap<LongOpenHashSet> chunkBuckets,
            ...
            this.boxes = boxes;
            this.complete = chunkBuckets.isEmpty();
            if (this.complete) {
                return;
            }
            // C方案: chunkRadius >= 0 时只纳入玩家附近范围内的桶;否则纳入所有桶(旧逻辑)
            for (Long2ObjectOpenHashMap.Entry<LongOpenHashSet> entry : chunkBuckets.long2ObjectEntrySet()) {
                long chunkKey = entry.getLongKey();
                if (chunkRadius >= 0) {
                    int bucketChunkX = chunkKeyX(chunkKey);
                    int bucketChunkZ = chunkKeyZ(chunkKey);
                ...
                BucketCursor cursor = new BucketCursor(
                        chunkKey,
                        bucket,
                        centerX,
                        centerY,
                ...
                if (cursor.hasNext()) {
                    this.pending.add(new IndexNode(cursor));
                }
            }
            this.complete = this.pending.isEmpty();
        }

        @Override
        public long peekDistanceSqr() {
            IndexNode node = this.pending.peek();
            return node == null ? Long.MAX_VALUE : node.peekDistanceSqr();
        }

        @Override
        public boolean next(BlockPos.MutableBlockPos target) {
            while (!this.pending.isEmpty()) {
                IndexNode node = this.pending.poll();
                long posKey = node.nextPosition();
                // 重新入队,若桶内仍有元素
                if (node.cursor.hasNext()) {
                    this.pending.add(node);
                }
                int x = BlockPos.getX(posKey);
                int y = BlockPos.getY(posKey);
                int z = BlockPos.getZ(posKey);
                // 去重(理论上索引内不会重复,但跨桶边界做保险)
                if (!this.emitted.add(posKey)) {
                    continue;
                }
                if (!containsAny(this.boxes, x, y, z)) {
                    continue;
                }
                target.set(x, y, z);
                return true;
        ...
        private static final class IndexNode implements Comparable<IndexNode> {
            private final BucketCursor cursor;

            private IndexNode(BucketCursor cursor) {
                this.cursor = cursor;
            }

            long peekDistanceSqr() {
                return this.cursor.peekDistanceSqr();
            }

            long nextPosition() {
                return this.cursor.nextPosition();
            }

            @Override
            public int compareTo(IndexNode other) {
                return Long.compare(this.peekDistanceSqr(), other.peekDistanceSqr());
            }
        }

        /**
         * 单个区块桶内的距离游标.遍历桶内所有位置,按到玩家的距离排序输出.
         * 桶通常较小(每区块至多4096个非空气位置,实际远小于此),使用简单数组排序即可.
         */
        private static final class BucketCursor {
            private final long[] positions;
            private final long[] distances;
            private int index;

            private BucketCursor(
                    long chunkKey,
                ...
                this.positions = new long[bucket.size()];
                this.distances = new long[bucket.size()];
                int i = 0;
                LongIterator iterator = bucket.iterator();
                while (iterator.hasNext()) {
                    long posKey = iterator.nextLong();
                    this.positions[i] = posKey;
                    int x = BlockPos.getX(posKey);
                    int y = BlockPos.getY(posKey);
                    int z = BlockPos.getZ(posKey);
                    long dx = x - (long) centerX;
                    long dy = y - (long) centerY;
                    long dz = z - (long) centerZ;
            ...
            private void sortIndices() {
                // 简单选择排序对桶内位置按距离排序(桶规模小,O(n^2) 可接受)
                for (int i = 0; i < this.positions.length - 1; i++) {
                    int best = i;
                    long bestDist = this.distances[i];
                    for (int j = i + 1; j < this.positions.length; j++) {
                        if (this.distances[j] < bestDist) {
                            best = j;
                            bestDist = this.distances[j];
                        }
                    }
                    if (best != i) {
                        long tmpPos = this.positions[i];
                        this.positions[i] = this.positions[best];
                        this.positions[best] = tmpPos;
                        long tmpDist = this.distances[i];
                        this.distances[i] = this.distances[best];
                        this.distances[best] = tmpDist;
                    }
                }
            }
    ...
    private static final class PlayerDistanceCursor implements DistanceCursor {
        private final List<PrinterBox> boxes;
