...
import me.aleksilassila.litematica.printer.handler.scan.SchematicBlockIndex;
...
public abstract class Module extends ConfigUtils {
    ...
    @Nullable
    private List<PrinterBox> lastPlacementBoxes;
    ...
    public void tick(TickContext context) {
        ...
        // 确保原理图非空气方块索引已构建,供 ScanCache 按距离遍历稀疏原理图
        SchematicBlockIndex.INSTANCE.ensureBuilt(schematic);
        ...
        // 诊断心跳：每 100 tick (~5秒) 输出模块运行状态, 定位"找不到候选"的原因
        if (isEnable() && context.gameTime % 100 == 0) {
            try {
                ...
                me.aleksilassila.litematica.printer.Reference.LOGGER.warn("[PrinterDiag] 心跳异常", e);
            }
        }
    ...
    private void wakeForSchematicChange(@Nullable WorldSchematic schematic) {
        ...
        if (!this.schematicIdentityInitialized) {
            ...
            changed = true;
        } else if (this.lastSchematicIdentity != schematic) {
            this.lastSchematicIdentity = schematic;
            changed = true;
        }
        // 检测 placement 列表变化(取消渲染、重新渲染、加载新原理图等)
        if (schematic != null) {
            List<PrinterBox> currentBoxes = LitematicaUtils.createSchematicPlacementBoxes();
            if (this.lastPlacementBoxes == null || !this.lastPlacementBoxes.equals(currentBoxes)) {
                this.lastPlacementBoxes = currentBoxes;
                changed = true;
            }
        }
        if (changed) {
            SchematicBlockIndex.INSTANCE.invalidate();
